	...
	public class CustomShaderOutput  : OutputGenerator
	{
		...
		public static IEnumerator Apply(CoordRect rect, Terrain terrain, object dataBox, Func<float,bool> stop= null)
		{
			...
			for (int i=0; i<textures.Length; i++)
			{
				...
				if (colors[i] != null) tex.SetPixels(0,0,tex.width,tex.height,colors[i]);
				tex.Apply();
				textures[i] = tex;
				yield return null;
			}
			...
			if (Preview.previewOutput == null && MapMagic.instance.customTerrainMaterial != null)
			{
				...
				#if CTS_PRESENT
				if (mode==Mode.CTS && ctsProfile != null && terrain.basemapDistance != ctsProfile.m_globalBasemapDistance)
		...
		static public void UpdateCustomShaderMaterials()
		{
			...
			//apply profile
			#if CTS_PRESENT
			if (CustomShaderOutput.mode == CustomShaderOutput.Mode.CTS && CustomShaderOutput.ctsProfile != null)
				CustomShaderOutput.CTS_UpdateShader(CustomShaderOutput.ctsProfile, MapMagic.instance.customTerrainMaterial);
			#endif
			...
			{
				case CustomShaderOutput.Mode.CTS: texIds = new int[4]; for (int t=0; t<texIds.Length; t++) texIds[t] = Shader.PropertyToID("_Texture_Splat_" + (t+1)); break;
				case CustomShaderOutput.Mode.RTP: texIds = new int[3]; for (int t=0; t<texIds.Length; t++) texIds[t] = Shader.PropertyToID("_Control_" + (t+1)); break;
				case CustomShaderOutput.Mode.MegaSplat: texIds = new int[2]; texIds[0] = Shader.PropertyToID("_SplatControl"); texIds[1] = Shader.PropertyToID("_SplatParams"); break;
