...
namespace MapMagic 
{
	...
	public class Matrix3<T>
	{
		...
		*/
		...
		public Matrix3 (CoordCube cube, T[] array=null)
		{
			this.cube = cube;
		...
		public Matrix3 (CoordDir offset, CoordDir size, T[] array=null)
		{
		...
		public Matrix3 (int x, int y, int z, T[] array=null)
		{
		...
		public Matrix3 (int ox, int oy, int oz, int sx, int sy, int sz, T[] array=null)
		{
		...
		public Matrix2<T> Matrix2 
		{get{ 
			#if WDEBUG
			if (cube.size.y!=1) Debug.LogError("Trying to convert non-flat Matrix3 to Matrix2");
			#endif
			return new Matrix2<T>( new CoordRect(cube.offset.x, cube.offset.z, cube.size.x, cube.size.z), array);
		}}
