...
namespace MapMagic
{
	...
	public class GeneratorsAsset : ScriptableObject , ISerializationCallbackReceiver
	{
		...
		#region Prepare/Calculate/Apply
			...
			{
				var changedTypes = new HashSet<Type>(); //to process output types
			...
			public void Generate (CoordRect rect, Chunk.Results results, Chunk.Size terrainSize, int seed, ref HashSet<System.Type> changedTypes, Func<float,bool> stop = null)
			{
				...
				if (!gen.enabled) continue;
					...
					if (biome.data==null) continue; 
						...
						{
							var innerGen = biome.data.list[ig];
						...
						//generate biome
						var maskBox = biome.mask.GetObject(results);
			...
			public void Process (CoordRect rect, Chunk.Results results, GeneratorsAsset gens, Chunk.Size terrainSize, HashSet<System.Type> changedTypes, Func<float,bool> stop = null)
			{
				...
				var processFunctionsCache = new Dictionary<Type, Action<CoordRect, Chunk.Results, GeneratorsAsset, Chunk.Size, Func<float,bool>>>();
			...
			processFunctionsCache.Add(typeof(TreesOutput), TreesOutput.Process); //adding trees and objects functions now just to avoid adding them in the next block
				...
				{
					var type = outGen.GetType();
				if (!processFunctionsCache.ContainsKey(type))
					{
						var o = outGen as OutputGenerator;
				...
				foreach (Type type in changedTypes)
				{
					...
					var processAction = processFunctionsCache[type];
			...
			{
				var e = Apply(rect,results,terrain,stop,purge);
			while (e.MoveNext());
			}
			...
			{
				//filling static methods hashes
				var applyFunctionsCache = new Dictionary<Type, Func<CoordRect, Terrain, object, Func<float,bool>, IEnumerator>>();
			...
			applyFunctionsCache.Add(typeof(TreesOutput), TreesOutput.Apply); 
				...
				results.nonEmpty.CheckAdd(type);
					...
					var applyFn = applyFunctionsCache[type];
		...
		#region Generators fns
			...
			public void OnDisableGenerator (Generator gen) //called by window AFTER generator has been disabled
			{
				...
				if (gen is OutputGenerator)
				{
					...
					if (!hasGenType)
					{
						var outGen = gen as OutputGenerator;


					if (mapMagic != null && mapMagic is MapMagic && MapMagic.instance != null)
						{
							var purgeAction = outGen.GetPurge();
		...
		{
			var gen = (Generator)System.Activator.CreateInstance(type);
...
//				{ Debug.LogError("MapMagic: Trying to add Output Generator while it already present in generators list"); return null; }
		...
		public Generator[] SmartDuplicateGenerators (Generator gen, bool appendToList=true) //generator duplicate in an array (or multiple gens if group. Or all gens if null)
		{
			...
			else if (gen is Group)
			{
				...
				var gensList = new List<Generator>();
				gensList.Add(gen);
				for (int g=0; g<list.Length; g++)
				...
				var copyGensHash = new HashSet<Generator>();
			...
			{
				var copyGen = (Generator)CustomSerialization.DeepCopy(gen);
		...
		public void ExportGenerator (Generator gen, Vector2 pos, string path=null) //if path is defined it will export generator using it
		{
			...
			var saveGens = SmartDuplicateGenerators(gen, appendToList:false);
		...
		public Generator[] ImportGenerator (Vector2 pos, bool appendToList=true)
		{
			...
			var loadedGens = (Generator[])CustomSerialization.ReadClass(0, classes, objects, floats, references);
		...
		{
			var gens = ScriptableObject.CreateInstance<GeneratorsAsset>();

			//creating initial generators
			var noiseGen = (NoiseGenerator182)gens.CreateGenerator(typeof(NoiseGenerator182), new Vector2(50,50));
			noiseGen.high = 0.9f;

			var curveGen = (CurveGenerator)gens.CreateGenerator(typeof(CurveGenerator), new Vector2(250,50));
			curveGen.curve = new AnimationCurve( new Keyframe[] { new Keyframe(0,0,0,0), new Keyframe(1,1,2.5f,1) } );

			var heightOut = (HeightOutput)gens.CreateGenerator(typeof(HeightOutput), new Vector2(450,50));
		...
		public static GeneratorsAsset DefaultVoxeland ()
		{
			...
			//creating initial generators
			var noiseGen = (NoiseGenerator2)gens.CreateGenerator(typeof(NoiseGenerator2), new Vector2(50,50));
			noiseGen.high = 0.9f;
		...
		#region Serialization
			...
			var objectsList = new List<UnityEngine.Object>();
			var floatsList = new List<float>();
			references.Clear();
			...
			public void OnAfterDeserialize ()
			{
				...
				var classesList = new List<string>(); classesList.AddRange(classes);
				var objectsList = new List<UnityEngine.Object>(); objectsList.AddRange(objects);
				var floatsList = new List<float>(); floatsList.AddRange(floats);
