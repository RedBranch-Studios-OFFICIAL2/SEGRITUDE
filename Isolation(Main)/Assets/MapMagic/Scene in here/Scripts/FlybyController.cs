...
namespace MapMagicDemo
{
	...
	public class FlybyController : MonoBehaviour 
	{
		...
		{
			var dir = target-start; float dist = dir.magnitude;
			var hitData = new RaycastHit();
		...
		{
			var lowestPoint = pos + new Vector3(0,2000000,0);
			...
			for (int a=0; a<angleSteps; a++)
			{
				...
				for (int d=0; d<distSteps; d++)
				{
					...
					var point = new Vector3( Mathf.Sin(currentAngle*Mathf.Deg2Rad), 0, Mathf.Cos(currentAngle*Mathf.Deg2Rad) ) * currentDist + pos;
					point.y = pos.y + 1000;

					var fPoint = SphereCast(point, point-new Vector3(0,1000,0), sphereRadius);
		...
		{
			var start = -direction*(distance/2);
		...
		public IEnumerable<Vector3> RectPositions2D (Vector3 direction, float distanceAlong, float stepsAlong, float distanceAcross, float stepsAcross) //linear
		{
			...
			var startAlong = -direction*(distanceAlong/2);
			var startAcross = -perpendicular*(distanceAcross/2);


			for (int i=0; i<stepsAlong; i++)
			{
				var pos = startAlong + direction*stepDistAlong*i;
		...
		{
			var lowestPoint = pos; lowestPoint.y = pos.y + 20000;
			...
			foreach (Vector3 spokeDir in SpokeDirections2D(dir, fov, 7))
			{
				...
				{
					var point = pos + spokeDir*(dist/2 + 50) + lineDir;
					...
					var rayPoint = RayCast(point+Vector3.up*2000, point+Vector3.down*1000);
					if (rayPoint.y > lowestPoint.y-radius) continue;

					var spherePoint = SphereCast(point+Vector3.up*2000, point+Vector3.down*1000, radius);
		...
		void Update ()
		{
			...
			var pos = transform.localPosition + moveVector*Time.deltaTime*speed;

			//if it is under terrain
			var flooredPos = RayCast(pos+new Vector3(0,2000,0), pos-new Vector3(0,2000,0)) + Vector3.up*2f;
